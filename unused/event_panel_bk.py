...
class EventPanel__(TextWrap, EditorBase, InterfaceData):# original
    ...
    def goal(self, value):
        self._goal = value
        self.set_game_event(GameEvent(
            name=f"goal{self.level}",
            title="Congratulation!",
            body=f"your population has reached {self.goal} people. Well Done, level {self.level} completed! travel to next level ?",
            end_text="",
    ...
    def update(self):
        ...
        # if eval(f"self.parent.player.{self.goal}"):
        #     if not f"goal{self.level}" in self.obsolete_events:
        #         self.set_game_event(self.game_events[f"goal{self.level}"])
...
class EventPanel(TextWrap, EditorBase):# new
    """Main functionalities:
    The EventPanel class is responsible for creating and managing game events in the GUI. It displays event information,
    such as the title, body, and end text, as well as any functions associated with it. The class also generates
    the body of the event based on a randomly selected planet and a deal offered by its alien population.
    ...
    def accept(self):
        if not self._hidden:
            if self.functions:
                ...
                #
                # else:
                getattr(global_params.app, self.functions["yes"])()
