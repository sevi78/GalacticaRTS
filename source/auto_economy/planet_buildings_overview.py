...
class PlanetBuildingsOverview(EditorBase):
    """
        ...
        for button in self.buttons:
            if button.name in building_factory.get_all_possible_categories():
                ...
                for building in possible_buildings:
                    ...
                    # draw building_score rect
                    # print (f"economy_calculator.building_production_scores: {economy_calculator.building_production_scores_sums}")
                    # print (f"economy_calculator.building_production_scores: {economy_calculator.get_lowest_score_keys(economy_calculator.building_production_scores_sums)}")

                    if self.planet:
                        best_buildings = self.planet.economy_agent.lowest_production_score_keys
                        color = pygame.color.THECOLORS.get("red") if building in best_buildings else pygame.color.THECOLORS.get("orange")
                        chart_width = 100 - self.planet.economy_agent.building_production_scores_sums[building]
    ...
    def draw(self):
        if not self._hidden and not self._disabled:
            ...
            # draw population
            if self.planet:
                self.draw_text(self.world_x + self.text_spacing, self.world_y + TOP_SPACING + self.text_spacing * 2, 200, self.title_font_size, f"buildings/slots: {len(self.planet.economy_agent.buildings)}/{self.planet.economy_agent.buildings_max}")
                self.draw_text(self.world_x + self.text_spacing, self.world_y + TOP_SPACING + self.text_spacing * 3, 200, self.title_font_size, f"production: : ")
                # for index,(key,  value) in enumerate(self.planet.economy_agent.production.items()):
                #     x = self.world_x + x + self.text_spacing
                #
                #
                #     self.draw_text(x, self.world_y + TOP_SPACING + self.text_spacing * 4, 200, self.title_font_size, f"{self.planet.economy_agent.production[key]}")
                #
                #     x += button_size
                ...
                for i in self.buttons:
                    x = i.world_x + 5
                    key = i.name
                    if key in self.planet.economy_agent.production.keys():
                        if not key == "population":
                            self.draw_text(x, self.world_y + TOP_SPACING + self.text_spacing * 4, 200, self.title_font_size - 4, f"{round(self.planet.economy_agent.production[key], 1)}")
                        else:
                            self.draw_text(x, self.world_y + TOP_SPACING + self.text_spacing * 4, 200, self.title_font_size - 4, f"{round(self.planet.economy_agent.production[key])}/{format_number(self.planet.economy_agent.population, 1)}/{format_number(self.planet.economy_agent.population_limit, 1)}")

                        # make visible based on posible resources
                        [i for i in self.buttons if i.name == key][
                            0]._hidden = not key in self.planet.economy_agent.possible_resources
