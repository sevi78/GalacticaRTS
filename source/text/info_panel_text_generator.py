...
#
#
# class InfoPanelTextGenerator:# old
#     def __init__(self):
...
#             else:
#                 text += f"reloader: " + str(None) + "\n"
#
#             text += f"move_stop: " + str(ship.move_stop) + "\n"
#             text += f"moving: " + str(ship.moving) + "\n"
#             text += f"position, x,y:" + str(int(ship.get_screen_x())) + "/" + str(int(ship.get_screen_y()))
#
#             if ship.target:
...
#             if self.target:
#                 text += f"reload_max_distance{ship.reload_max_distance}/{math.dist((self.world_x, self.world_y),(self.target.world_x, self.target.world_y))}\n"
#
#             else:
...
class InfoPanelTextGenerator:
    ...
    def create_info_panel_planet_text(self, planet):
        ...
        for resource in planet.possible_resources:
            text += f"- {resource}\n"
        if planet.orbit_object:
            distance = math.dist((planet.world_x, planet.world_y), (planet.orbit_object.world_x, planet.orbit_object.world_y))
    ...
    def create_create_info_panel_level_text(self, level: int, data: dict) -> str:
        ...
        try:
            ...
            goal += "no goal defined!!\n"
        width, height = (format_number(data["globals"]["width"] * 1000, 1), format_number(data["globals"]["height"] * 1000, 1))
        num_planets = sum(1 for obj in data["celestial_objects"].values() if obj["type"] == "planet")
        num_moons = sum(1 for obj in data["celestial_objects"].values() if obj["type"] == "moon")
        num_suns = sum(1 for obj in data["celestial_objects"].values() if obj["type"] == "sun")
        resources = set()
