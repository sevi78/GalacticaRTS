...
class WeaponSelectHandler:
    ...
    def select_weapon(self, obj, weapon_name):
        self.current_weapon_selection = weapon_name
        self.editor.update()
        self.update_obj(obj)

    def upgrade(self, obj):
        if self.editor.current_weapon["level"] < self.max_weapons_upgrade_level:
            prices = building_factory.get_prices_from_weapons_dict(
                    self.editor.current_weapon["name"], self.editor.current_weapon["level"])
            building_factory.build(self.editor.current_weapon["name"], obj, prices=prices)
            self.update_obj(obj)
        else:
    ...
    def update_obj(self, obj):
        obj.weapon_handler.current_weapon_selection = self.current_weapon_selection

        # if obj has selected weapon already, upgrade it
        if obj.weapon_handler.current_weapon_selection in obj.weapon_handler.weapons.keys():
            obj.weapon_handler.current_weapon = copy.deepcopy(
                    obj.weapon_handler.weapons[obj.weapon_handler.current_weapon_selection])
            self.editor.current_weapon = copy.deepcopy(self.all_weapons[self.current_weapon_selection])
            self.editor.current_weapon["level"] = copy.deepcopy(
                    obj.weapon_handler.weapons[obj.weapon_handler.current_weapon_selection]["level"])
            self.editor.upgrade_button.set_text("upgrade")
            self.editor.upgrade_button.tooltip = "upgrade"
        else:
            obj.weapon_handler.current_weapon = copy.deepcopy(self.all_weapons[self.current_weapon_selection])
            self.editor.current_weapon = copy.deepcopy(self.all_weapons[self.current_weapon_selection])
            self.editor.upgrade_button.set_text("buy")
            self.editor.upgrade_button.tooltip = "buy"
        self.editor.update()
...
class WeaponSelect(EditorBase):
    def __init__(self, win, x, y, width, height, is_sub_widget=False, **kwargs):
        ...
        self.weapon_select_handler = WeaponSelectHandler(self)
    ...
    def create_buttons(self):
        ...
        for key in self.all_weapons.keys():
            ...
            icon = ImageButton(win=self.win,
                    ...
                    include_text=True,
                    layer=self.layer,
                    on_click=lambda weapon_=key: self.weapon_select_handler.select_weapon(self.obj, weapon_),
                    name=key,
        ...
        self.upgrade_button = ImageButton(win=self.win,
                ...
                include_text=True,
                layer=self.layer,
                on_click=lambda: self.weapon_select_handler.upgrade(None),
                name="upgrade_button",
                text="upgrade",
                text_color=self.frame_color,
                font_size=BUTTON_FONT_SIZE,
    ...
    def obj(self, value):
        ...
        if self._obj:
            ...
            self.weapon_select_handler.update_obj(self._obj)
            self.update()

    # def update_obj(self):
    #     self.obj.weapon_handler.current_weapon_selection = self.current_weapon_selection
    #
    #     # if obj has selected weapon already, upgrade it
    #     if self.obj.weapon_handler.current_weapon_selection in self.obj.weapon_handler.weapons.keys():
    #         self.obj.weapon_handler.current_weapon = copy.deepcopy(
    #                 self.obj.weapon_handler.weapons[self.obj.weapon_handler.current_weapon_selection])
    #         self.current_weapon = copy.deepcopy(self.all_weapons[self.current_weapon_selection])
    #         self.current_weapon["level"] = copy.deepcopy(
    #                 self.obj.weapon_handler.weapons[self.obj.weapon_handler.current_weapon_selection]["level"])
    ...
    def draw_weapon_prices(self):
        ...
        if level + 1 > self.weapon_select_handler.max_weapons_upgrade_level:
    ...
    def draw(self):
        if not self._hidden and not self._disabled:
            self.weapon_select_handler.update_obj(self.obj)
            self.update()
            self.draw_image()
            self.draw_frame()
