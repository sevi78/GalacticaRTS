...
class BoxSelection:
    ...
    def draw(self):
        ...
        if self.start_pos is not None and self.end_pos is not None and pygame.mouse.get_pressed()[0]:
            ...
            pygame.draw.rect(self.window, self.color, (
                rect_pos,
                rect_size), config.ui_rounded_corner_small_thickness, config.ui_rounded_corner_radius_small)
    ...
    def check_selection(self):
        ...
        pygame.Rect object based on the start and end positions of the selection, and then checks if each object's screen
        position is within the selection rectangle. The selected objects are stored in a list and returned.
        """
    ...
    def select_objects(self):
        ...
        if len(selected_planets) > 0:
            config.app.set_selected_planet(selected_planets[0])
    ...
    def listen(self, events):
        ...
        if config.hover_object:
            if not hasattr(config.hover_object, "property") or hasattr(config.hover_object, "drag_enabled"):
                return

        self.draw()
        # event_text.text = f"config.hover_object: {config.hover_object}"
        for event in events:
